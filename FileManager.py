#Import Package for os manipulations
import os
...
def get_listFile(path):
    ...
    #Boucle pour parcourir le dossier et retourner chaque fichier du dossier
...
def graphe(listAbscisse : list, listOrdonnee : list, pays : list):
...
if __name__=="__main__":
    ...
    for article in articles.keys():
         ...
         # enregistrement des données pour le tableau de bord
         for elt in liste_pays:
            T_pays.append(elt)
    #Affichage des données
    print_data(articles)
    

abscisse, ordonnee = graphe(abscisse, ordonnee, T_pays)

pyplot.xlabel('Country')
pyplot.ylabel('Article publié')
pyplot.title('Presentatation du flux d_article par pays')
pyplot.bar(abscisse,ordonnee, color='#9711ff')
#pyplot.show()
